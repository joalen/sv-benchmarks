...
typedef struct Obj {
    int field;
    klee_make_symbolic(&field, sizeof(int), "field");
} Obj;
...
typedef struct WorkStealQueue {
    ...
    long MaxSize;
    klee_make_symbolic(&MaxSize, sizeof(long), "MaxSize");
    long InitialSize; // must be a power of 2
    klee_make_symbolic(&InitialSize, sizeof(long), "InitialSize");

    long head;  // only updated by Take
    klee_make_symbolic(&head, sizeof(long), "head");
    long tail;  // only updated by Push and Pop
    klee_make_symbolic(&tail, sizeof(long), "tail");
