...
// with type: int __cpufreq_driver_target(struct cpufreq_policy *policy, unsigned int target_freq, unsigned int relation)
...
// with type: int cpufreq_register_governor(struct cpufreq_governor *governor)
// with return type: int
int __VERIFIER_nondet_int(void);
...
// with type: void cpufreq_unregister_governor(struct cpufreq_governor *governor)
