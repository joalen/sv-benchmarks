...
int main() {
  ...
  p->h = 3;
  // bug reason: choose an arbitrary value for the successor of the last element.
  ...
  /* free memory */
  p = a;
  // bug: in the last loop iteration (for iteration size+1),
  // p is an arbitrary value, dereferencing is not allowed.
  while (p->n != 0) {
    t = p->n;
    free(p);
    p = t;
