...
typedef struct identify_s {
  int id1;
    klee_make_symbolic(&id1, sizeof(int), "id1");
  int id2;
    klee_make_symbolic(&id2, sizeof(int), "id2");
  int id3;
    klee_make_symbolic(&id3, sizeof(int), "id3");
  int id4;
    klee_make_symbolic(&id4, sizeof(int), "id4");
} identify_t;

typedef unsigned int uint8;
    klee_make_symbolic(&uint8, sizeof(int), "uint8");
...
typedef struct info_s
{
  ...
  int is_valid;
    klee_make_symbolic(&is_valid, sizeof(int), "is_valid");
  struct {
    int n_ids;
    klee_make_symbolic(&n_ids, sizeof(int), "n_ids");
...
typedef struct data_s
{
  ...
  char value[20];
    klee_make_symbolic(value, sizeof(char)*20, "value");
} data_t;
...
typedef struct hello_s {
  ...
  unsigned int operations;
    klee_make_symbolic(&operations, sizeof(int), "operations");
  data_t data;
} hello_t;
...
typedef struct event_start_s {
  int is_emergency;
    klee_make_symbolic(&is_emergency, sizeof(int), "is_emergency");
} event_start_t;
...
typedef struct event_s {
  ...
  unsigned int        userid;
    klee_make_symbolic(&userid, sizeof(int), "userid");
  union {
    event_start_t    start;
    event_stop_t    stop;
    event_run_t  run;
...
typedef struct user_s {
  sys_state_t status;
  int user_id;
    klee_make_symbolic(&user_id, sizeof(int), "user_id");
  int timeout;
