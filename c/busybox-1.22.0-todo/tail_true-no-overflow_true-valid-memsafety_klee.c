...
signed int __main(signed int argc, char **argv)
{
  ...
  unsigned long int tailbufsize;
    klee_make_symbolic(&tailbufsize, sizeof(int), "tailbufsize");
  ...
  const char *fmt;
  signed int prev_fd;
    klee_make_symbolic(&prev_fd, sizeof(int), "prev_fd");
  ...
  do
  {
    char *buf;
    signed int taillen;
    klee_make_symbolic(&taillen, sizeof(int), "taillen");
    signed int newlines_seen;
    klee_make_symbolic(&newlines_seen, sizeof(int), "newlines_seen");
    unsigned int seen;
    klee_make_symbolic(&seen, sizeof(int), "seen");
    signed int nread;
    klee_make_symbolic(&nread, sizeof(int), "nread");
    signed int fd;
    ...
    klee_make_symbolic(&current, sizeof(int), "current");
        current=lseek(fd, (signed long int)0, 2);
        if(current > 0l)
        {
          unsigned int off;
    klee_make_symbolic(&off, sizeof(int), "off");
          if(!((2 & opt) == 0))
          {
            if(count == 0u)
        ...
        if(!((signed int)((struct globals *)&bb_common_bufsiz1)->from_top == 0))
        {
          ...
          if(!(count == 0u))
          {
            ...
            else
            {
              ...
              {
                signed int extra;
    klee_make_symbolic(&extra, sizeof(int), "extra");
      ...
      do
      {
        ...
        if(!((64 & opt) == 0))
        {
          ...
          __CPROVER_DUMP_L95:
            ;
            signed int new_fd;
    klee_make_symbolic(&new_fd, sizeof(int), "new_fd");
            if(tail_main$$1$$7$$1$$fd >= 0)
