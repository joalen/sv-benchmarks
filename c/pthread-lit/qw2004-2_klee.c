...
volatile int stoppingFlag;
    klee_make_symbolic(&stoppingFlag, sizeof(int), "stoppingFlag");
volatile int pendingIo;
    klee_make_symbolic(&pendingIo, sizeof(int), "pendingIo");
volatile int stoppingEvent;
    klee_make_symbolic(&stoppingEvent, sizeof(int), "stoppingEvent");
volatile int stopped;
    klee_make_symbolic(&stopped, sizeof(int), "stopped");
...
void BCSP_IoDecrement() {
    int pending;
    klee_make_symbolic(&pending, sizeof(int), "pending");
    pending = dec();
    if (pending == 0) {
    __VERIFIER_atomic_begin();
	stoppingEvent = 1;
