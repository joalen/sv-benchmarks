#include <klee/klee.h>
int __return_main;
    klee_make_symbolic(&__return_main, sizeof(int), "__return_main");
void abort(void);
extern void __assert_fail(const char *, const char *, unsigned int, const char *) __attribute__ ((__nothrow__ , __leaf__)) __attribute__ ((__noreturn__));
void reach_error() { __assert_fail("0", "pals_lcr.6.1.ufo.BOUNDED-12.pals.c.p+lhb-reducer.c", 4, "reach_error"); }
_Bool __VERIFIER_nondet_bool();
char __VERIFIER_nondet_char();
void assert(_Bool arg);
void abort(void);
void assume_abort_if_not(int cond) {
  if(!cond) {abort();}
}
typedef char msg_t;
    klee_make_symbolic(&msg_t, sizeof(char), "msg_t");
typedef int port_t;
    klee_make_symbolic(&port_t, sizeof(int), "port_t");
void read(port_t p, msg_t m);
void write(port_t p, msg_t m);
msg_t nomsg = (msg_t )-1;
char r1 = '\x0';
port_t p1 = 0;
char p1_old = '\x0';
char p1_new = '\x0';
char id1 = '\x0';
char st1 = '\x0';
msg_t send1 = '\x0';
_Bool mode1 = 0;
port_t p2 = 0;
char p2_old = '\x0';
char p2_new = '\x0';
char id2 = '\x0';
char st2 = '\x0';
msg_t send2 = '\x0';
_Bool mode2 = 0;
port_t p3 = 0;
char p3_old = '\x0';
char p3_new = '\x0';
char id3 = '\x0';
char st3 = '\x0';
msg_t send3 = '\x0';
_Bool mode3 = 0;
port_t p4 = 0;
char p4_old = '\x0';
char p4_new = '\x0';
char id4 = '\x0';
char st4 = '\x0';
msg_t send4 = '\x0';
_Bool mode4 = 0;
port_t p5 = 0;
char p5_old = '\x0';
char p5_new = '\x0';
char id5 = '\x0';
char st5 = '\x0';
msg_t send5 = '\x0';
_Bool mode5 = 0;
port_t p6 = 0;
char p6_old = '\x0';
char p6_new = '\x0';
char id6 = '\x0';
char st6 = '\x0';
msg_t send6 = '\x0';
_Bool mode6 = 0;
void node1();
void node2();
void node3();
void node4();
void node5();
void node6();
void (*nodes[6])() = { &node1, &node2, &node3, &node4, &node5, &node6 };
int init();
int check();
int main();
int __return_1676;
    klee_make_symbolic(&__return_1676, sizeof(int), "__return_1676");
int __return_1799;
    klee_make_symbolic(&__return_1799, sizeof(int), "__return_1799");
int __return_1915;
    klee_make_symbolic(&__return_1915, sizeof(int), "__return_1915");
int __return_2031;
    klee_make_symbolic(&__return_2031, sizeof(int), "__return_2031");
int __return_2147;
    klee_make_symbolic(&__return_2147, sizeof(int), "__return_2147");
int __tmp_2156_0;
    klee_make_symbolic(&__tmp_2156_0, sizeof(int), "__tmp_2156_0");
int __return_2265;
    klee_make_symbolic(&__return_2265, sizeof(int), "__return_2265");
int __return_2160;
    klee_make_symbolic(&__return_2160, sizeof(int), "__return_2160");
 int main()
 {
 int main__c1;
    klee_make_symbolic(&main__c1, sizeof(int), "main__c1");
 int main__i2;
    klee_make_symbolic(&main__i2, sizeof(int), "main__i2");
 main__c1 = 0;
 r1 = __VERIFIER_nondet_char();
 id1 = __VERIFIER_nondet_char();
 st1 = __VERIFIER_nondet_char();
 send1 = __VERIFIER_nondet_char();
 mode1 = __VERIFIER_nondet_bool();
 id2 = __VERIFIER_nondet_char();
 st2 = __VERIFIER_nondet_char();
 send2 = __VERIFIER_nondet_char();
 mode2 = __VERIFIER_nondet_bool();
 id3 = __VERIFIER_nondet_char();
 st3 = __VERIFIER_nondet_char();
 send3 = __VERIFIER_nondet_char();
 mode3 = __VERIFIER_nondet_bool();
 id4 = __VERIFIER_nondet_char();
 st4 = __VERIFIER_nondet_char();
 send4 = __VERIFIER_nondet_char();
 mode4 = __VERIFIER_nondet_bool();
 id5 = __VERIFIER_nondet_char();
 st5 = __VERIFIER_nondet_char();
 send5 = __VERIFIER_nondet_char();
 mode5 = __VERIFIER_nondet_bool();
 id6 = __VERIFIER_nondet_char();
 st6 = __VERIFIER_nondet_char();
 send6 = __VERIFIER_nondet_char();
 mode6 = __VERIFIER_nondet_bool();
 {
 int init__tmp;
    klee_make_symbolic(&init__tmp, sizeof(int), "init__tmp");
 if (((int)r1) == 0)
 {
 if (((int)id1) >= 0)
 {
 if (((int)st1) == 0)
 {
 if (((int)send1) == ((int)id1))
 {
 if (((int)mode1) == 0)
 {
 if (((int)id2) >= 0)
 {
 if (((int)st2) == 0)
 {
 if (((int)send2) == ((int)id2))
 {
 if (((int)mode2) == 0)
 {
 if (((int)id3) >= 0)
 {
 if (((int)st3) == 0)
 {
 if (((int)send3) == ((int)id3))
 {
 if (((int)mode3) == 0)
 {
 if (((int)id4) >= 0)
 {
 if (((int)st4) == 0)
 {
 if (((int)send4) == ((int)id4))
 {
 if (((int)mode4) == 0)
 {
 if (((int)id5) >= 0)
 {
 if (((int)st5) == 0)
 {
 if (((int)send5) == ((int)id5))
 {
 if (((int)mode5) == 0)
 {
 if (((int)id6) >= 0)
 {
 if (((int)st6) == 0)
 {
 if (((int)send6) == ((int)id6))
 {
 if (((int)mode6) == 0)
 {
 if (((int)id1) != ((int)id2))
 {
 if (((int)id1) != ((int)id3))
 {
 if (((int)id1) != ((int)id4))
 {
 if (((int)id1) != ((int)id5))
 {
 if (((int)id1) != ((int)id6))
 {
 if (((int)id2) != ((int)id3))
 {
 if (((int)id2) != ((int)id4))
 {
 if (((int)id2) != ((int)id5))
 {
 if (((int)id2) != ((int)id6))
 {
 if (((int)id3) != ((int)id4))
 {
 if (((int)id3) != ((int)id5))
 {
 if (((int)id3) != ((int)id6))
 {
 if (((int)id4) != ((int)id5))
 {
 if (((int)id4) != ((int)id6))
 {
 if (((int)id5) != ((int)id6))
 {
 init__tmp = 1;
 label_2902:; 
 label_2898:; 
 label_2894:; 
 label_2890:; 
 label_2886:; 
 label_2882:; 
 label_2878:; 
 label_2874:; 
 label_2870:; 
 label_2866:; 
 label_2862:; 
 label_2858:; 
 label_2854:; 
 label_2850:; 
 label_2846:; 
 label_2842:; 
 label_2838:; 
 label_2834:; 
 label_2830:; 
 label_2826:; 
 label_2822:; 
 label_2818:; 
 label_2814:; 
 label_2810:; 
 label_2806:; 
 label_2802:; 
 label_2798:; 
 label_2794:; 
 label_2790:; 
 label_2786:; 
 label_2782:; 
 label_2778:; 
 label_2774:; 
 label_2770:; 
 label_2766:; 
 label_2762:; 
 label_2758:; 
 label_2754:; 
 label_2750:; 
 label_1675:; 
  __return_1676 = init__tmp;
 main__i2 = __return_1676;
 if (main__i2 != 0)
 {
 p1_old = nomsg;
 p1_new = nomsg;
 p2_old = nomsg;
 p2_new = nomsg;
 p3_old = nomsg;
 p3_new = nomsg;
 p4_old = nomsg;
 p4_new = nomsg;
 p5_old = nomsg;
 p5_new = nomsg;
 p6_old = nomsg;
 p6_new = nomsg;
 main__i2 = 0;
 if (main__i2 < 12)
 {
 {
 msg_t node1__m1;
 node1__m1 = nomsg;
 if (!(mode1 == 0))
 {
 r1 = (char)(((int)r1) + 1);
 node1__m1 = p6_old;
 p6_old = nomsg;
 if (((int)node1__m1) != ((int)nomsg))
 {
 if (((int)node1__m1) > ((int)id1))
 {
 send1 = node1__m1;
 label_1705:; 
 mode1 = 0;
 label_1707:; 
 {
 msg_t node2__m2;
 node2__m2 = nomsg;
 if (!(mode2 == 0))
 {
 node2__m2 = p1_old;
 p1_old = nomsg;
 if (((int)node2__m2) != ((int)nomsg))
 {
 if (((int)node2__m2) > ((int)id2))
 {
 send2 = node2__m2;
 label_1719:; 
 mode2 = 0;
 label_1721:; 
 {
 msg_t node3__m3;
 node3__m3 = nomsg;
 if (!(mode3 == 0))
 {
 node3__m3 = p2_old;
 p2_old = nomsg;
 if (((int)node3__m3) != ((int)nomsg))
 {
 if (((int)node3__m3) > ((int)id3))
 {
 send3 = node3__m3;
 label_1733:; 
 mode3 = 0;
 label_1735:; 
 {
 msg_t node4__m4;
 node4__m4 = nomsg;
 if (!(mode4 == 0))
 {
 node4__m4 = p3_old;
 p3_old = nomsg;
 if (((int)node4__m4) != ((int)nomsg))
 {
 if (((int)node4__m4) > ((int)id4))
 {
 send4 = node4__m4;
 label_1747:; 
 mode4 = 0;
 label_1749:; 
 {
 msg_t node5__m5;
 node5__m5 = nomsg;
 if (!(mode5 == 0))
 {
 node5__m5 = p4_old;
 p4_old = nomsg;
 if (((int)node5__m5) != ((int)nomsg))
 {
 if (((int)node5__m5) > ((int)id5))
 {
 send5 = node5__m5;
 label_1761:; 
 mode5 = 0;
 label_1763:; 
 {
 msg_t node6__m6;
 node6__m6 = nomsg;
 if (!(mode6 == 0))
 {
 node6__m6 = p5_old;
 p5_old = nomsg;
 if (((int)node6__m6) != ((int)nomsg))
 {
 if (((int)node6__m6) > ((int)id6))
 {
 send6 = node6__m6;
 label_1775:; 
 mode6 = 0;
 label_1777:; 
 p1_old = p1_new;
 p1_new = nomsg;
 p2_old = p2_new;
 p2_new = nomsg;
 p3_old = p3_new;
 p3_new = nomsg;
 p4_old = p4_new;
 p4_new = nomsg;
 p5_old = p5_new;
 p5_new = nomsg;
 p6_old = p6_new;
 p6_new = nomsg;
 {
 int check__tmp;
    klee_make_symbolic(&check__tmp, sizeof(int), "check__tmp");
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) <= 1)
 {
 if (((int)r1) >= 6)
 {
 label_2659:; 
 if (((int)r1) < 6)
 {
 check__tmp = 1;
 label_2665:; 
 label_2658:; 
 label_1798:; 
  __return_1799 = check__tmp;
 main__c1 = __return_1799;
 {
 _Bool __tmp_1;
 __tmp_1 = main__c1;
 _Bool assert__arg;
 assert__arg = __tmp_1;
 if (assert__arg == 0)
 {
 return __return_main;
 }
 else 
 {
 int main____CPAchecker_TMP_0 = main__i2;
 main__i2 = main__i2 + 1;
 if (main__i2 < 12)
 {
 {
 msg_t node1__m1;
 node1__m1 = nomsg;
 if (!(mode1 == 0))
 {
 r1 = (char)(((int)r1) + 1);
 node1__m1 = p6_old;
 p6_old = nomsg;
 if (((int)node1__m1) != ((int)nomsg))
 {
 if (((int)node1__m1) > ((int)id1))
 {
 send1 = node1__m1;
 label_1821:; 
 mode1 = 0;
 label_1823:; 
 {
 msg_t node2__m2;
 node2__m2 = nomsg;
 if (!(mode2 == 0))
 {
 node2__m2 = p1_old;
 p1_old = nomsg;
 if (((int)node2__m2) != ((int)nomsg))
 {
 if (((int)node2__m2) > ((int)id2))
 {
 send2 = node2__m2;
 label_1835:; 
 mode2 = 0;
 label_1837:; 
 {
 msg_t node3__m3;
 node3__m3 = nomsg;
 if (!(mode3 == 0))
 {
 node3__m3 = p2_old;
 p2_old = nomsg;
 if (((int)node3__m3) != ((int)nomsg))
 {
 if (((int)node3__m3) > ((int)id3))
 {
 send3 = node3__m3;
 label_1849:; 
 mode3 = 0;
 label_1851:; 
 {
 msg_t node4__m4;
 node4__m4 = nomsg;
 if (!(mode4 == 0))
 {
 node4__m4 = p3_old;
 p3_old = nomsg;
 if (((int)node4__m4) != ((int)nomsg))
 {
 if (((int)node4__m4) > ((int)id4))
 {
 send4 = node4__m4;
 label_1863:; 
 mode4 = 0;
 label_1865:; 
 {
 msg_t node5__m5;
 node5__m5 = nomsg;
 if (!(mode5 == 0))
 {
 node5__m5 = p4_old;
 p4_old = nomsg;
 if (((int)node5__m5) != ((int)nomsg))
 {
 if (((int)node5__m5) > ((int)id5))
 {
 send5 = node5__m5;
 label_1877:; 
 mode5 = 0;
 label_1879:; 
 {
 msg_t node6__m6;
 node6__m6 = nomsg;
 if (!(mode6 == 0))
 {
 node6__m6 = p5_old;
 p5_old = nomsg;
 if (((int)node6__m6) != ((int)nomsg))
 {
 if (((int)node6__m6) > ((int)id6))
 {
 send6 = node6__m6;
 label_1891:; 
 mode6 = 0;
 label_1893:; 
 p1_old = p1_new;
 p1_new = nomsg;
 p2_old = p2_new;
 p2_new = nomsg;
 p3_old = p3_new;
 p3_new = nomsg;
 p4_old = p4_new;
 p4_new = nomsg;
 p5_old = p5_new;
 p5_new = nomsg;
 p6_old = p6_new;
 p6_new = nomsg;
 {
 int check__tmp;
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) <= 1)
 {
 if (((int)r1) >= 6)
 {
 label_2565:; 
 if (((int)r1) < 6)
 {
 check__tmp = 1;
 label_2571:; 
 label_2564:; 
 label_1914:; 
  __return_1915 = check__tmp;
 main__c1 = __return_1915;
 {
 _Bool __tmp_2;
 __tmp_2 = main__c1;
 _Bool assert__arg;
 assert__arg = __tmp_2;
 if (assert__arg == 0)
 {
 return __return_main;
 }
 else 
 {
 int main____CPAchecker_TMP_0 = main__i2;
 main__i2 = main__i2 + 1;
 if (main__i2 < 12)
 {
 {
 msg_t node1__m1;
 node1__m1 = nomsg;
 if (!(mode1 == 0))
 {
 r1 = (char)(((int)r1) + 1);
 node1__m1 = p6_old;
 p6_old = nomsg;
 if (((int)node1__m1) != ((int)nomsg))
 {
 if (((int)node1__m1) > ((int)id1))
 {
 send1 = node1__m1;
 label_1937:; 
 mode1 = 0;
 label_1939:; 
 {
 msg_t node2__m2;
 node2__m2 = nomsg;
 if (!(mode2 == 0))
 {
 node2__m2 = p1_old;
 p1_old = nomsg;
 if (((int)node2__m2) != ((int)nomsg))
 {
 if (((int)node2__m2) > ((int)id2))
 {
 send2 = node2__m2;
 label_1951:; 
 mode2 = 0;
 label_1953:; 
 {
 msg_t node3__m3;
 node3__m3 = nomsg;
 if (!(mode3 == 0))
 {
 node3__m3 = p2_old;
 p2_old = nomsg;
 if (((int)node3__m3) != ((int)nomsg))
 {
 if (((int)node3__m3) > ((int)id3))
 {
 send3 = node3__m3;
 label_1965:; 
 mode3 = 0;
 label_1967:; 
 {
 msg_t node4__m4;
 node4__m4 = nomsg;
 if (!(mode4 == 0))
 {
 node4__m4 = p3_old;
 p3_old = nomsg;
 if (((int)node4__m4) != ((int)nomsg))
 {
 if (((int)node4__m4) > ((int)id4))
 {
 send4 = node4__m4;
 label_1979:; 
 mode4 = 0;
 label_1981:; 
 {
 msg_t node5__m5;
 node5__m5 = nomsg;
 if (!(mode5 == 0))
 {
 node5__m5 = p4_old;
 p4_old = nomsg;
 if (((int)node5__m5) != ((int)nomsg))
 {
 if (((int)node5__m5) > ((int)id5))
 {
 send5 = node5__m5;
 label_1993:; 
 mode5 = 0;
 label_1995:; 
 {
 msg_t node6__m6;
 node6__m6 = nomsg;
 if (!(mode6 == 0))
 {
 node6__m6 = p5_old;
 p5_old = nomsg;
 if (((int)node6__m6) != ((int)nomsg))
 {
 if (((int)node6__m6) > ((int)id6))
 {
 send6 = node6__m6;
 label_2007:; 
 mode6 = 0;
 label_2009:; 
 p1_old = p1_new;
 p1_new = nomsg;
 p2_old = p2_new;
 p2_new = nomsg;
 p3_old = p3_new;
 p3_new = nomsg;
 p4_old = p4_new;
 p4_new = nomsg;
 p5_old = p5_new;
 p5_new = nomsg;
 p6_old = p6_new;
 p6_new = nomsg;
 {
 int check__tmp;
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) <= 1)
 {
 if (((int)r1) >= 6)
 {
 label_2471:; 
 if (((int)r1) < 6)
 {
 check__tmp = 1;
 label_2477:; 
 label_2470:; 
 label_2030:; 
  __return_2031 = check__tmp;
 main__c1 = __return_2031;
 {
 _Bool __tmp_3;
 __tmp_3 = main__c1;
 _Bool assert__arg;
 assert__arg = __tmp_3;
 if (assert__arg == 0)
 {
 return __return_main;
 }
 else 
 {
 int main____CPAchecker_TMP_0 = main__i2;
 main__i2 = main__i2 + 1;
 if (main__i2 < 12)
 {
 {
 msg_t node1__m1;
 node1__m1 = nomsg;
 if (!(mode1 == 0))
 {
 r1 = (char)(((int)r1) + 1);
 node1__m1 = p6_old;
 p6_old = nomsg;
 if (((int)node1__m1) != ((int)nomsg))
 {
 if (((int)node1__m1) > ((int)id1))
 {
 send1 = node1__m1;
 label_2053:; 
 mode1 = 0;
 label_2055:; 
 {
 msg_t node2__m2;
 node2__m2 = nomsg;
 if (!(mode2 == 0))
 {
 node2__m2 = p1_old;
 p1_old = nomsg;
 if (((int)node2__m2) != ((int)nomsg))
 {
 if (((int)node2__m2) > ((int)id2))
 {
 send2 = node2__m2;
 label_2067:; 
 mode2 = 0;
 label_2069:; 
 {
 msg_t node3__m3;
 node3__m3 = nomsg;
 if (!(mode3 == 0))
 {
 node3__m3 = p2_old;
 p2_old = nomsg;
 if (((int)node3__m3) != ((int)nomsg))
 {
 if (((int)node3__m3) > ((int)id3))
 {
 send3 = node3__m3;
 label_2081:; 
 mode3 = 0;
 label_2083:; 
 {
 msg_t node4__m4;
 node4__m4 = nomsg;
 if (!(mode4 == 0))
 {
 node4__m4 = p3_old;
 p3_old = nomsg;
 if (((int)node4__m4) != ((int)nomsg))
 {
 if (((int)node4__m4) > ((int)id4))
 {
 send4 = node4__m4;
 label_2095:; 
 mode4 = 0;
 label_2097:; 
 {
 msg_t node5__m5;
 node5__m5 = nomsg;
 if (!(mode5 == 0))
 {
 node5__m5 = p4_old;
 p4_old = nomsg;
 if (((int)node5__m5) != ((int)nomsg))
 {
 if (((int)node5__m5) > ((int)id5))
 {
 send5 = node5__m5;
 label_2109:; 
 mode5 = 0;
 label_2111:; 
 {
 msg_t node6__m6;
 node6__m6 = nomsg;
 if (!(mode6 == 0))
 {
 node6__m6 = p5_old;
 p5_old = nomsg;
 if (((int)node6__m6) != ((int)nomsg))
 {
 if (((int)node6__m6) > ((int)id6))
 {
 send6 = node6__m6;
 label_2123:; 
 mode6 = 0;
 label_2125:; 
 p1_old = p1_new;
 p1_new = nomsg;
 p2_old = p2_new;
 p2_new = nomsg;
 p3_old = p3_new;
 p3_new = nomsg;
 p4_old = p4_new;
 p4_new = nomsg;
 p5_old = p5_new;
 p5_new = nomsg;
 p6_old = p6_new;
 p6_new = nomsg;
 {
 int check__tmp;
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) <= 1)
 {
 if (((int)r1) >= 6)
 {
 label_2377:; 
 if (((int)r1) < 6)
 {
 check__tmp = 1;
 label_2383:; 
 label_2376:; 
 label_2146:; 
  __return_2147 = check__tmp;
 main__c1 = __return_2147;
 {
 _Bool __tmp_4;
 __tmp_4 = main__c1;
 _Bool assert__arg;
 assert__arg = __tmp_4;
 if (assert__arg == 0)
 {
 return __return_main;
 }
 else 
 {
 int main____CPAchecker_TMP_0 = main__i2;
 main__i2 = main__i2 + 1;
 __tmp_2156_0 = main____CPAchecker_TMP_0;
 label_2156:; 
 main____CPAchecker_TMP_0 = __tmp_2156_0;
 if (main__i2 < 12)
 {
 {
 msg_t node1__m1;
 node1__m1 = nomsg;
 if (!(mode1 == 0))
 {
 r1 = (char)(((int)r1) + 1);
 node1__m1 = p6_old;
 p6_old = nomsg;
 if (((int)node1__m1) != ((int)nomsg))
 {
 if (((int)node1__m1) > ((int)id1))
 {
 send1 = node1__m1;
 label_2171:; 
 mode1 = 0;
 label_2173:; 
 {
 msg_t node2__m2;
 node2__m2 = nomsg;
 if (!(mode2 == 0))
 {
 node2__m2 = p1_old;
 p1_old = nomsg;
 if (((int)node2__m2) != ((int)nomsg))
 {
 if (((int)node2__m2) > ((int)id2))
 {
 send2 = node2__m2;
 label_2185:; 
 mode2 = 0;
 label_2187:; 
 {
 msg_t node3__m3;
 node3__m3 = nomsg;
 if (!(mode3 == 0))
 {
 node3__m3 = p2_old;
 p2_old = nomsg;
 if (((int)node3__m3) != ((int)nomsg))
 {
 if (((int)node3__m3) > ((int)id3))
 {
 send3 = node3__m3;
 label_2199:; 
 mode3 = 0;
 label_2201:; 
 {
 msg_t node4__m4;
 node4__m4 = nomsg;
 if (!(mode4 == 0))
 {
 node4__m4 = p3_old;
 p3_old = nomsg;
 if (((int)node4__m4) != ((int)nomsg))
 {
 if (((int)node4__m4) > ((int)id4))
 {
 send4 = node4__m4;
 label_2213:; 
 mode4 = 0;
 label_2215:; 
 {
 msg_t node5__m5;
 node5__m5 = nomsg;
 if (!(mode5 == 0))
 {
 node5__m5 = p4_old;
 p4_old = nomsg;
 if (((int)node5__m5) != ((int)nomsg))
 {
 if (((int)node5__m5) > ((int)id5))
 {
 send5 = node5__m5;
 label_2227:; 
 mode5 = 0;
 label_2229:; 
 {
 msg_t node6__m6;
 node6__m6 = nomsg;
 if (!(mode6 == 0))
 {
 node6__m6 = p5_old;
 p5_old = nomsg;
 if (((int)node6__m6) != ((int)nomsg))
 {
 if (((int)node6__m6) > ((int)id6))
 {
 send6 = node6__m6;
 label_2241:; 
 mode6 = 0;
 label_2243:; 
 p1_old = p1_new;
 p1_new = nomsg;
 p2_old = p2_new;
 p2_new = nomsg;
 p3_old = p3_new;
 p3_new = nomsg;
 p4_old = p4_new;
 p4_new = nomsg;
 p5_old = p5_new;
 p5_new = nomsg;
 p6_old = p6_new;
 p6_new = nomsg;
 {
 int check__tmp;
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) <= 1)
 {
 if (((int)r1) >= 6)
 {
 label_2283:; 
 if (((int)r1) < 6)
 {
 check__tmp = 1;
 label_2289:; 
 label_2282:; 
 label_2264:; 
  __return_2265 = check__tmp;
 main__c1 = __return_2265;
 {
 _Bool __tmp_5;
 __tmp_5 = main__c1;
 _Bool assert__arg;
 assert__arg = __tmp_5;
 if (assert__arg == 0)
 {
 {reach_error();}
 return __return_main;
 }
 else 
 {
 int main____CPAchecker_TMP_0 = main__i2;
 main__i2 = main__i2 + 1;
 __tmp_2156_0 = main____CPAchecker_TMP_0;
 goto label_2156;
 }
 }
 }
 else 
 {
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) == 1)
 {
 check__tmp = 1;
 goto label_2289;
 }
 else 
 {
 check__tmp = 0;
 goto label_2289;
 }
 }
 }
 else 
 {
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) == 0)
 {
 goto label_2283;
 }
 else 
 {
 check__tmp = 0;
 goto label_2282;
 }
 }
 }
 else 
 {
 check__tmp = 0;
 goto label_2264;
 }
 }
 }
 else 
 {
 if (((int)node6__m6) == ((int)id6))
 {
 st6 = 1;
 goto label_2241;
 }
 else 
 {
 goto label_2241;
 }
 }
 }
 else 
 {
 goto label_2241;
 }
 }
 else 
 {
 int node6____CPAchecker_TMP_0;
    klee_make_symbolic(&node6____CPAchecker_TMP_0, sizeof(int), "node6____CPAchecker_TMP_0");
 if (send6 != nomsg)
 {
 if (p6_new == nomsg)
 {
 node6____CPAchecker_TMP_0 = send6;
 label_2301:; 
 p6_new = node6____CPAchecker_TMP_0;
 mode6 = 1;
 goto label_2243;
 }
 else 
 {
 label_2300:; 
 node6____CPAchecker_TMP_0 = p6_new;
 goto label_2301;
 }
 }
 else 
 {
 goto label_2300;
 }
 }
 }
 }
 else 
 {
 if (((int)node5__m5) == ((int)id5))
 {
 st5 = 1;
 goto label_2227;
 }
 else 
 {
 goto label_2227;
 }
 }
 }
 else 
 {
 goto label_2227;
 }
 }
 else 
 {
 int node5____CPAchecker_TMP_0;
    klee_make_symbolic(&node5____CPAchecker_TMP_0, sizeof(int), "node5____CPAchecker_TMP_0");
 if (send5 != nomsg)
 {
 if (p5_new == nomsg)
 {
 node5____CPAchecker_TMP_0 = send5;
 label_2314:; 
 p5_new = node5____CPAchecker_TMP_0;
 mode5 = 1;
 goto label_2229;
 }
 else 
 {
 label_2313:; 
 node5____CPAchecker_TMP_0 = p5_new;
 goto label_2314;
 }
 }
 else 
 {
 goto label_2313;
 }
 }
 }
 }
 else 
 {
 if (((int)node4__m4) == ((int)id4))
 {
 st4 = 1;
 goto label_2213;
 }
 else 
 {
 goto label_2213;
 }
 }
 }
 else 
 {
 goto label_2213;
 }
 }
 else 
 {
 int node4____CPAchecker_TMP_0;
    klee_make_symbolic(&node4____CPAchecker_TMP_0, sizeof(int), "node4____CPAchecker_TMP_0");
 if (send4 != nomsg)
 {
 if (p4_new == nomsg)
 {
 node4____CPAchecker_TMP_0 = send4;
 label_2327:; 
 p4_new = node4____CPAchecker_TMP_0;
 mode4 = 1;
 goto label_2215;
 }
 else 
 {
 label_2326:; 
 node4____CPAchecker_TMP_0 = p4_new;
 goto label_2327;
 }
 }
 else 
 {
 goto label_2326;
 }
 }
 }
 }
 else 
 {
 if (((int)node3__m3) == ((int)id3))
 {
 st3 = 1;
 goto label_2199;
 }
 else 
 {
 goto label_2199;
 }
 }
 }
 else 
 {
 goto label_2199;
 }
 }
 else 
 {
 int node3____CPAchecker_TMP_0;
    klee_make_symbolic(&node3____CPAchecker_TMP_0, sizeof(int), "node3____CPAchecker_TMP_0");
 if (send3 != nomsg)
 {
 if (p3_new == nomsg)
 {
 node3____CPAchecker_TMP_0 = send3;
 label_2340:; 
 p3_new = node3____CPAchecker_TMP_0;
 mode3 = 1;
 goto label_2201;
 }
 else 
 {
 label_2339:; 
 node3____CPAchecker_TMP_0 = p3_new;
 goto label_2340;
 }
 }
 else 
 {
 goto label_2339;
 }
 }
 }
 }
 else 
 {
 if (((int)node2__m2) == ((int)id2))
 {
 st2 = 1;
 goto label_2185;
 }
 else 
 {
 goto label_2185;
 }
 }
 }
 else 
 {
 goto label_2185;
 }
 }
 else 
 {
 int node2____CPAchecker_TMP_0;
    klee_make_symbolic(&node2____CPAchecker_TMP_0, sizeof(int), "node2____CPAchecker_TMP_0");
 if (send2 != nomsg)
 {
 if (p2_new == nomsg)
 {
 node2____CPAchecker_TMP_0 = send2;
 label_2353:; 
 p2_new = node2____CPAchecker_TMP_0;
 mode2 = 1;
 goto label_2187;
 }
 else 
 {
 label_2352:; 
 node2____CPAchecker_TMP_0 = p2_new;
 goto label_2353;
 }
 }
 else 
 {
 goto label_2352;
 }
 }
 }
 }
 else 
 {
 if (((int)node1__m1) == ((int)id1))
 {
 st1 = 1;
 goto label_2171;
 }
 else 
 {
 send1 = node1__m1;
 goto label_2171;
 }
 }
 }
 else 
 {
 goto label_2171;
 }
 }
 else 
 {
 int node1____CPAchecker_TMP_0;
    klee_make_symbolic(&node1____CPAchecker_TMP_0, sizeof(int), "node1____CPAchecker_TMP_0");
 if (send1 != nomsg)
 {
 if (p1_new == nomsg)
 {
 node1____CPAchecker_TMP_0 = send1;
 label_2367:; 
 p1_new = node1____CPAchecker_TMP_0;
 mode1 = 1;
 goto label_2173;
 }
 else 
 {
 label_2366:; 
 node1____CPAchecker_TMP_0 = p1_new;
 goto label_2367;
 }
 }
 else 
 {
 goto label_2366;
 }
 }
 }
 }
 else 
 {
  __return_2160 = 0;
 return __return_2160;
 }
 }
 }
 }
 else 
 {
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) == 1)
 {
 check__tmp = 1;
 goto label_2383;
 }
 else 
 {
 check__tmp = 0;
 goto label_2383;
 }
 }
 }
 else 
 {
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) == 0)
 {
 goto label_2377;
 }
 else 
 {
 check__tmp = 0;
 goto label_2376;
 }
 }
 }
 else 
 {
 check__tmp = 0;
 goto label_2146;
 }
 }
 }
 else 
 {
 if (((int)node6__m6) == ((int)id6))
 {
 st6 = 1;
 goto label_2123;
 }
 else 
 {
 goto label_2123;
 }
 }
 }
 else 
 {
 goto label_2123;
 }
 }
 else 
 {
 int node6____CPAchecker_TMP_0;
 if (send6 != nomsg)
 {
 if (p6_new == nomsg)
 {
 node6____CPAchecker_TMP_0 = send6;
 label_2395:; 
 p6_new = node6____CPAchecker_TMP_0;
 mode6 = 1;
 goto label_2125;
 }
 else 
 {
 label_2394:; 
 node6____CPAchecker_TMP_0 = p6_new;
 goto label_2395;
 }
 }
 else 
 {
 goto label_2394;
 }
 }
 }
 }
 else 
 {
 if (((int)node5__m5) == ((int)id5))
 {
 st5 = 1;
 goto label_2109;
 }
 else 
 {
 goto label_2109;
 }
 }
 }
 else 
 {
 goto label_2109;
 }
 }
 else 
 {
 int node5____CPAchecker_TMP_0;
 if (send5 != nomsg)
 {
 if (p5_new == nomsg)
 {
 node5____CPAchecker_TMP_0 = send5;
 label_2408:; 
 p5_new = node5____CPAchecker_TMP_0;
 mode5 = 1;
 goto label_2111;
 }
 else 
 {
 label_2407:; 
 node5____CPAchecker_TMP_0 = p5_new;
 goto label_2408;
 }
 }
 else 
 {
 goto label_2407;
 }
 }
 }
 }
 else 
 {
 if (((int)node4__m4) == ((int)id4))
 {
 st4 = 1;
 goto label_2095;
 }
 else 
 {
 goto label_2095;
 }
 }
 }
 else 
 {
 goto label_2095;
 }
 }
 else 
 {
 int node4____CPAchecker_TMP_0;
 if (send4 != nomsg)
 {
 if (p4_new == nomsg)
 {
 node4____CPAchecker_TMP_0 = send4;
 label_2421:; 
 p4_new = node4____CPAchecker_TMP_0;
 mode4 = 1;
 goto label_2097;
 }
 else 
 {
 label_2420:; 
 node4____CPAchecker_TMP_0 = p4_new;
 goto label_2421;
 }
 }
 else 
 {
 goto label_2420;
 }
 }
 }
 }
 else 
 {
 if (((int)node3__m3) == ((int)id3))
 {
 st3 = 1;
 goto label_2081;
 }
 else 
 {
 goto label_2081;
 }
 }
 }
 else 
 {
 goto label_2081;
 }
 }
 else 
 {
 int node3____CPAchecker_TMP_0;
 if (send3 != nomsg)
 {
 if (p3_new == nomsg)
 {
 node3____CPAchecker_TMP_0 = send3;
 label_2434:; 
 p3_new = node3____CPAchecker_TMP_0;
 mode3 = 1;
 goto label_2083;
 }
 else 
 {
 label_2433:; 
 node3____CPAchecker_TMP_0 = p3_new;
 goto label_2434;
 }
 }
 else 
 {
 goto label_2433;
 }
 }
 }
 }
 else 
 {
 if (((int)node2__m2) == ((int)id2))
 {
 st2 = 1;
 goto label_2067;
 }
 else 
 {
 goto label_2067;
 }
 }
 }
 else 
 {
 goto label_2067;
 }
 }
 else 
 {
 int node2____CPAchecker_TMP_0;
 if (send2 != nomsg)
 {
 if (p2_new == nomsg)
 {
 node2____CPAchecker_TMP_0 = send2;
 label_2447:; 
 p2_new = node2____CPAchecker_TMP_0;
 mode2 = 1;
 goto label_2069;
 }
 else 
 {
 label_2446:; 
 node2____CPAchecker_TMP_0 = p2_new;
 goto label_2447;
 }
 }
 else 
 {
 goto label_2446;
 }
 }
 }
 }
 else 
 {
 if (((int)node1__m1) == ((int)id1))
 {
 st1 = 1;
 goto label_2053;
 }
 else 
 {
 send1 = node1__m1;
 goto label_2053;
 }
 }
 }
 else 
 {
 goto label_2053;
 }
 }
 else 
 {
 int node1____CPAchecker_TMP_0;
 if (send1 != nomsg)
 {
 if (p1_new == nomsg)
 {
 node1____CPAchecker_TMP_0 = send1;
 label_2461:; 
 p1_new = node1____CPAchecker_TMP_0;
 mode1 = 1;
 goto label_2055;
 }
 else 
 {
 label_2460:; 
 node1____CPAchecker_TMP_0 = p1_new;
 goto label_2461;
 }
 }
 else 
 {
 goto label_2460;
 }
 }
 }
 }
 else 
 {
 return __return_main;
 }
 }
 }
 }
 else 
 {
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) == 1)
 {
 check__tmp = 1;
 goto label_2477;
 }
 else 
 {
 check__tmp = 0;
 goto label_2477;
 }
 }
 }
 else 
 {
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) == 0)
 {
 goto label_2471;
 }
 else 
 {
 check__tmp = 0;
 goto label_2470;
 }
 }
 }
 else 
 {
 check__tmp = 0;
 goto label_2030;
 }
 }
 }
 else 
 {
 if (((int)node6__m6) == ((int)id6))
 {
 st6 = 1;
 goto label_2007;
 }
 else 
 {
 goto label_2007;
 }
 }
 }
 else 
 {
 goto label_2007;
 }
 }
 else 
 {
 int node6____CPAchecker_TMP_0;
 if (send6 != nomsg)
 {
 if (p6_new == nomsg)
 {
 node6____CPAchecker_TMP_0 = send6;
 label_2489:; 
 p6_new = node6____CPAchecker_TMP_0;
 mode6 = 1;
 goto label_2009;
 }
 else 
 {
 label_2488:; 
 node6____CPAchecker_TMP_0 = p6_new;
 goto label_2489;
 }
 }
 else 
 {
 goto label_2488;
 }
 }
 }
 }
 else 
 {
 if (((int)node5__m5) == ((int)id5))
 {
 st5 = 1;
 goto label_1993;
 }
 else 
 {
 goto label_1993;
 }
 }
 }
 else 
 {
 goto label_1993;
 }
 }
 else 
 {
 int node5____CPAchecker_TMP_0;
 if (send5 != nomsg)
 {
 if (p5_new == nomsg)
 {
 node5____CPAchecker_TMP_0 = send5;
 label_2502:; 
 p5_new = node5____CPAchecker_TMP_0;
 mode5 = 1;
 goto label_1995;
 }
 else 
 {
 label_2501:; 
 node5____CPAchecker_TMP_0 = p5_new;
 goto label_2502;
 }
 }
 else 
 {
 goto label_2501;
 }
 }
 }
 }
 else 
 {
 if (((int)node4__m4) == ((int)id4))
 {
 st4 = 1;
 goto label_1979;
 }
 else 
 {
 goto label_1979;
 }
 }
 }
 else 
 {
 goto label_1979;
 }
 }
 else 
 {
 int node4____CPAchecker_TMP_0;
 if (send4 != nomsg)
 {
 if (p4_new == nomsg)
 {
 node4____CPAchecker_TMP_0 = send4;
 label_2515:; 
 p4_new = node4____CPAchecker_TMP_0;
 mode4 = 1;
 goto label_1981;
 }
 else 
 {
 label_2514:; 
 node4____CPAchecker_TMP_0 = p4_new;
 goto label_2515;
 }
 }
 else 
 {
 goto label_2514;
 }
 }
 }
 }
 else 
 {
 if (((int)node3__m3) == ((int)id3))
 {
 st3 = 1;
 goto label_1965;
 }
 else 
 {
 goto label_1965;
 }
 }
 }
 else 
 {
 goto label_1965;
 }
 }
 else 
 {
 int node3____CPAchecker_TMP_0;
 if (send3 != nomsg)
 {
 if (p3_new == nomsg)
 {
 node3____CPAchecker_TMP_0 = send3;
 label_2528:; 
 p3_new = node3____CPAchecker_TMP_0;
 mode3 = 1;
 goto label_1967;
 }
 else 
 {
 label_2527:; 
 node3____CPAchecker_TMP_0 = p3_new;
 goto label_2528;
 }
 }
 else 
 {
 goto label_2527;
 }
 }
 }
 }
 else 
 {
 if (((int)node2__m2) == ((int)id2))
 {
 st2 = 1;
 goto label_1951;
 }
 else 
 {
 goto label_1951;
 }
 }
 }
 else 
 {
 goto label_1951;
 }
 }
 else 
 {
 int node2____CPAchecker_TMP_0;
 if (send2 != nomsg)
 {
 if (p2_new == nomsg)
 {
 node2____CPAchecker_TMP_0 = send2;
 label_2541:; 
 p2_new = node2____CPAchecker_TMP_0;
 mode2 = 1;
 goto label_1953;
 }
 else 
 {
 label_2540:; 
 node2____CPAchecker_TMP_0 = p2_new;
 goto label_2541;
 }
 }
 else 
 {
 goto label_2540;
 }
 }
 }
 }
 else 
 {
 if (((int)node1__m1) == ((int)id1))
 {
 st1 = 1;
 goto label_1937;
 }
 else 
 {
 send1 = node1__m1;
 goto label_1937;
 }
 }
 }
 else 
 {
 goto label_1937;
 }
 }
 else 
 {
 int node1____CPAchecker_TMP_0;
 if (send1 != nomsg)
 {
 if (p1_new == nomsg)
 {
 node1____CPAchecker_TMP_0 = send1;
 label_2555:; 
 p1_new = node1____CPAchecker_TMP_0;
 mode1 = 1;
 goto label_1939;
 }
 else 
 {
 label_2554:; 
 node1____CPAchecker_TMP_0 = p1_new;
 goto label_2555;
 }
 }
 else 
 {
 goto label_2554;
 }
 }
 }
 }
 else 
 {
 return __return_main;
 }
 }
 }
 }
 else 
 {
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) == 1)
 {
 check__tmp = 1;
 goto label_2571;
 }
 else 
 {
 check__tmp = 0;
 goto label_2571;
 }
 }
 }
 else 
 {
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) == 0)
 {
 goto label_2565;
 }
 else 
 {
 check__tmp = 0;
 goto label_2564;
 }
 }
 }
 else 
 {
 check__tmp = 0;
 goto label_1914;
 }
 }
 }
 else 
 {
 if (((int)node6__m6) == ((int)id6))
 {
 st6 = 1;
 goto label_1891;
 }
 else 
 {
 goto label_1891;
 }
 }
 }
 else 
 {
 goto label_1891;
 }
 }
 else 
 {
 int node6____CPAchecker_TMP_0;
 if (send6 != nomsg)
 {
 if (p6_new == nomsg)
 {
 node6____CPAchecker_TMP_0 = send6;
 label_2583:; 
 p6_new = node6____CPAchecker_TMP_0;
 mode6 = 1;
 goto label_1893;
 }
 else 
 {
 label_2582:; 
 node6____CPAchecker_TMP_0 = p6_new;
 goto label_2583;
 }
 }
 else 
 {
 goto label_2582;
 }
 }
 }
 }
 else 
 {
 if (((int)node5__m5) == ((int)id5))
 {
 st5 = 1;
 goto label_1877;
 }
 else 
 {
 goto label_1877;
 }
 }
 }
 else 
 {
 goto label_1877;
 }
 }
 else 
 {
 int node5____CPAchecker_TMP_0;
 if (send5 != nomsg)
 {
 if (p5_new == nomsg)
 {
 node5____CPAchecker_TMP_0 = send5;
 label_2596:; 
 p5_new = node5____CPAchecker_TMP_0;
 mode5 = 1;
 goto label_1879;
 }
 else 
 {
 label_2595:; 
 node5____CPAchecker_TMP_0 = p5_new;
 goto label_2596;
 }
 }
 else 
 {
 goto label_2595;
 }
 }
 }
 }
 else 
 {
 if (((int)node4__m4) == ((int)id4))
 {
 st4 = 1;
 goto label_1863;
 }
 else 
 {
 goto label_1863;
 }
 }
 }
 else 
 {
 goto label_1863;
 }
 }
 else 
 {
 int node4____CPAchecker_TMP_0;
 if (send4 != nomsg)
 {
 if (p4_new == nomsg)
 {
 node4____CPAchecker_TMP_0 = send4;
 label_2609:; 
 p4_new = node4____CPAchecker_TMP_0;
 mode4 = 1;
 goto label_1865;
 }
 else 
 {
 label_2608:; 
 node4____CPAchecker_TMP_0 = p4_new;
 goto label_2609;
 }
 }
 else 
 {
 goto label_2608;
 }
 }
 }
 }
 else 
 {
 if (((int)node3__m3) == ((int)id3))
 {
 st3 = 1;
 goto label_1849;
 }
 else 
 {
 goto label_1849;
 }
 }
 }
 else 
 {
 goto label_1849;
 }
 }
 else 
 {
 int node3____CPAchecker_TMP_0;
 if (send3 != nomsg)
 {
 if (p3_new == nomsg)
 {
 node3____CPAchecker_TMP_0 = send3;
 label_2622:; 
 p3_new = node3____CPAchecker_TMP_0;
 mode3 = 1;
 goto label_1851;
 }
 else 
 {
 label_2621:; 
 node3____CPAchecker_TMP_0 = p3_new;
 goto label_2622;
 }
 }
 else 
 {
 goto label_2621;
 }
 }
 }
 }
 else 
 {
 if (((int)node2__m2) == ((int)id2))
 {
 st2 = 1;
 goto label_1835;
 }
 else 
 {
 goto label_1835;
 }
 }
 }
 else 
 {
 goto label_1835;
 }
 }
 else 
 {
 int node2____CPAchecker_TMP_0;
 if (send2 != nomsg)
 {
 if (p2_new == nomsg)
 {
 node2____CPAchecker_TMP_0 = send2;
 label_2635:; 
 p2_new = node2____CPAchecker_TMP_0;
 mode2 = 1;
 goto label_1837;
 }
 else 
 {
 label_2634:; 
 node2____CPAchecker_TMP_0 = p2_new;
 goto label_2635;
 }
 }
 else 
 {
 goto label_2634;
 }
 }
 }
 }
 else 
 {
 if (((int)node1__m1) == ((int)id1))
 {
 st1 = 1;
 goto label_1821;
 }
 else 
 {
 send1 = node1__m1;
 goto label_1821;
 }
 }
 }
 else 
 {
 goto label_1821;
 }
 }
 else 
 {
 int node1____CPAchecker_TMP_0;
 if (send1 != nomsg)
 {
 if (p1_new == nomsg)
 {
 node1____CPAchecker_TMP_0 = send1;
 label_2649:; 
 p1_new = node1____CPAchecker_TMP_0;
 mode1 = 1;
 goto label_1823;
 }
 else 
 {
 label_2648:; 
 node1____CPAchecker_TMP_0 = p1_new;
 goto label_2649;
 }
 }
 else 
 {
 goto label_2648;
 }
 }
 }
 }
 else 
 {
 return __return_main;
 }
 }
 }
 }
 else 
 {
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) == 1)
 {
 check__tmp = 1;
 goto label_2665;
 }
 else 
 {
 check__tmp = 0;
 goto label_2665;
 }
 }
 }
 else 
 {
 if ((((((((int)st1) + ((int)st2)) + ((int)st3)) + ((int)st4)) + ((int)st5)) + ((int)st6)) == 0)
 {
 goto label_2659;
 }
 else 
 {
 check__tmp = 0;
 goto label_2658;
 }
 }
 }
 else 
 {
 check__tmp = 0;
 goto label_1798;
 }
 }
 }
 else 
 {
 if (((int)node6__m6) == ((int)id6))
 {
 st6 = 1;
 goto label_1775;
 }
 else 
 {
 goto label_1775;
 }
 }
 }
 else 
 {
 goto label_1775;
 }
 }
 else 
 {
 int node6____CPAchecker_TMP_0;
 if (send6 != nomsg)
 {
 if (p6_new == nomsg)
 {
 node6____CPAchecker_TMP_0 = send6;
 label_2677:; 
 p6_new = node6____CPAchecker_TMP_0;
 mode6 = 1;
 goto label_1777;
 }
 else 
 {
 label_2676:; 
 node6____CPAchecker_TMP_0 = p6_new;
 goto label_2677;
 }
 }
 else 
 {
 goto label_2676;
 }
 }
 }
 }
 else 
 {
 if (((int)node5__m5) == ((int)id5))
 {
 st5 = 1;
 goto label_1761;
 }
 else 
 {
 goto label_1761;
 }
 }
 }
 else 
 {
 goto label_1761;
 }
 }
 else 
 {
 int node5____CPAchecker_TMP_0;
 if (send5 != nomsg)
 {
 if (p5_new == nomsg)
 {
 node5____CPAchecker_TMP_0 = send5;
 label_2690:; 
 p5_new = node5____CPAchecker_TMP_0;
 mode5 = 1;
 goto label_1763;
 }
 else 
 {
 label_2689:; 
 node5____CPAchecker_TMP_0 = p5_new;
 goto label_2690;
 }
 }
 else 
 {
 goto label_2689;
 }
 }
 }
 }
 else 
 {
 if (((int)node4__m4) == ((int)id4))
 {
 st4 = 1;
 goto label_1747;
 }
 else 
 {
 goto label_1747;
 }
 }
 }
 else 
 {
 goto label_1747;
 }
 }
 else 
 {
 int node4____CPAchecker_TMP_0;
 if (send4 != nomsg)
 {
 if (p4_new == nomsg)
 {
 node4____CPAchecker_TMP_0 = send4;
 label_2703:; 
 p4_new = node4____CPAchecker_TMP_0;
 mode4 = 1;
 goto label_1749;
 }
 else 
 {
 label_2702:; 
 node4____CPAchecker_TMP_0 = p4_new;
 goto label_2703;
 }
 }
 else 
 {
 goto label_2702;
 }
 }
 }
 }
 else 
 {
 if (((int)node3__m3) == ((int)id3))
 {
 st3 = 1;
 goto label_1733;
 }
 else 
 {
 goto label_1733;
 }
 }
 }
 else 
 {
 goto label_1733;
 }
 }
 else 
 {
 int node3____CPAchecker_TMP_0;
 if (send3 != nomsg)
 {
 if (p3_new == nomsg)
 {
 node3____CPAchecker_TMP_0 = send3;
 label_2716:; 
 p3_new = node3____CPAchecker_TMP_0;
 mode3 = 1;
 goto label_1735;
 }
 else 
 {
 label_2715:; 
 node3____CPAchecker_TMP_0 = p3_new;
 goto label_2716;
 }
 }
 else 
 {
 goto label_2715;
 }
 }
 }
 }
 else 
 {
 if (((int)node2__m2) == ((int)id2))
 {
 st2 = 1;
 goto label_1719;
 }
 else 
 {
 goto label_1719;
 }
 }
 }
 else 
 {
 goto label_1719;
 }
 }
 else 
 {
 int node2____CPAchecker_TMP_0;
 if (send2 != nomsg)
 {
 if (p2_new == nomsg)
 {
 node2____CPAchecker_TMP_0 = send2;
 label_2729:; 
 p2_new = node2____CPAchecker_TMP_0;
 mode2 = 1;
 goto label_1721;
 }
 else 
 {
 label_2728:; 
 node2____CPAchecker_TMP_0 = p2_new;
 goto label_2729;
 }
 }
 else 
 {
 goto label_2728;
 }
 }
 }
 }
 else 
 {
 if (((int)node1__m1) == ((int)id1))
 {
 st1 = 1;
 goto label_1705;
 }
 else 
 {
 send1 = node1__m1;
 goto label_1705;
 }
 }
 }
 else 
 {
 goto label_1705;
 }
 }
 else 
 {
 int node1____CPAchecker_TMP_0;
 if (send1 != nomsg)
 {
 if (p1_new == nomsg)
 {
 node1____CPAchecker_TMP_0 = send1;
 label_2743:; 
 p1_new = node1____CPAchecker_TMP_0;
 mode1 = 1;
 goto label_1707;
 }
 else 
 {
 label_2742:; 
 node1____CPAchecker_TMP_0 = p1_new;
 goto label_2743;
 }
 }
 else 
 {
 goto label_2742;
 }
 }
 }
 }
 else 
 {
 return __return_main;
 }
 }
 else 
 {
 return __return_main;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2902;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2898;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2894;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2890;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2886;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2882;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2878;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2874;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2870;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2866;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2862;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2858;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2854;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2850;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2846;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2842;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2838;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2834;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2830;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2826;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2822;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2818;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2814;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2810;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2806;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2802;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2798;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2794;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2790;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2786;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2782;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2778;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2774;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2770;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2766;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2762;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2758;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2754;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_2750;
 }
 }
 else 
 {
 init__tmp = 0;
 goto label_1675;
 }
 }
 }
